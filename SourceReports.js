...
function SourceReports() {
    ...
    const data = useMemo(() => {
        const grouped = records.reduce((acc, record) => {
            const { source, leadName } = record;
            if (!acc[source]) {
                acc[source] = [];
            }
            acc[source].push(leadName);
            return acc;
        }, {});

        return Object.entries(grouped).map(([source, leadNames]) => ({
            source,
            leadNames,
        }));
    }, [records]);
  ...
  <h1 style={{ color: "black", textAlign: "center", fontFamily: "serif", marginRight: "20px" }}>LEAD NAME BY SOURCE</h1>
