...
function ServiceGraph() {
  ...
  const leadserviceCounts = records.reduce((acc, record) => {
    acc[record.service] = (acc[record.service] || 0) + 1;
    return acc;
  }, {});
  console.log(leadserviceCounts)

  const data = Object.keys(leadserviceCounts).map(service => ({
    name: service,
    value: leadserviceCounts[service]
  }));
  ...
  <h1 style={{ color: "black", textAlign: "center", fontFamily: "serif", marginRight: "20px" }}>Lead Service Graphs</h1>
